...
class EmployeeController extends AbstractController
{
                                 ...
                                 SaleRepository $saleRepository, Request $request)
    {
        ...
        $employees = $userRepository->findEmployees($this->store);
        ...
        foreach ($employees as $employee){
            ...
            $fees = $employeeFeeRepository
                ->groupByPeriodDate($model['start'],$model['end'],$employee,$this->store);
                                  ...
                                  EntityManagerInterface $entityManager): RedirectResponse
    {
        ...
        if ($attendance === null){
            ...
            $attendance->setStore($this->store);
        }else{
            $attendance->setStatus($request->get('statut'));
        }
                               ...
                               AttendanceRepository $attendanceRepository)
    {
        ...
        $attendances = $attendanceRepository
            ->findByMonthYear($model['monthNow'],$model['year'],$employee,$this->store);
                        ...
                        EmployeeFeeRepository $employeeFeeRepository)
    {
        ...
        if ($customer !== null){
            ...
            $model['products'] = $productRepository
                ->saleByCustomer($customer,$model['start'],$model['end'],$this->store);
                                  ...
                                  EntityManagerInterface $entityManager): RedirectResponse
    {
        ...
        $salaryPayments = $salaryPaymentRepository
            ->findByMonthYear((int) $request->get('month'),
                (int) $request->get('year'), $employee,$this->store);
        ...
        if (($salaryPaid + (int) $request->get('amount')) <= $salaryLied){
            ...
            $salaryPayment->setStore($this->store);
