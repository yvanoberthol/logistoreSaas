...
class ExpenseController extends AbstractController
{
                          ...
                          ExpenseRepository $expenseRepository)
    {
            ...
            if ($_POST['type'] !== '0'){
                ...
                $model['expenses'] = $expenseRepository
                    ->findExpenseByPeriod($model['start'],$model['end'],
                        (int) $model['typeSearch'],$this->store);
            }else{
                $model['expenses'] = $expenseRepository
                    ->findExpenseByPeriod($model['start'],$model['end'],$this->store);
            }
        }else{
            $model['expenses'] = $expenseRepository
                ->findExpenseByPeriod($model['start'],$model['end'],$this->store);
        ...
        $model['types'] = $typeRepository->findBy(['status' => true,'store' => $this->store]);
