...
class FileController extends AbstractController
{
                                   ...
                                   EncashmentService $encashmentService): Response
    {
        ...
        $encashment = $encashmentRepository->findByDate($date,$employee,$this->store);

        $file = $encashmentService
            ->getInventory($model['date'],$model['date'],$employee,true,$this->store);
        ...
        $model['customers'] = $customerService
            ->getCredits($model['date'],$model['date'],
                $customerRepository->findBy(['store'=> $this->store]),$employee);
                           ...
                           EntityManagerInterface $entityManager): Response
    {
        ...
        if ($request->isMethod('POST')){
            if ($request->get('_property') === 'initialBalance'){
                ...
                $encashment = $encashmentRepository->findByDate($date,$employee,$this->store) ?? new Encashment();

                $encashment->setDate($addDate);
                $encashment->setEmployee($employee);
                $encashment->setStore($this->store);
