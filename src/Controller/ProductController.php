...
class ProductController extends AbstractController
{
                          ...
                          ProductService $productService)
    {
        if (empty($request->get('category'))){
            $products = $productRepository
                ->findBy(['store' => $this->store],['addDate' => 'DESC']);
        }else{
        ...
        if ($request->get('status') === 'byStockAlert'){
            ...
            $model['products'] = $productService->getProductByStockAlert($products,$this->store);
        }elseif ($request->get('status') === 'byNew'){
                $model['search'] = 'byNew';
                $model['products'] = array_filter($productService->countStocks($products,$this->store),
                    static function (Product $product){
                    return $product->isNew();
                });
        }elseif ($request->get('status') === 'byOutOfStock'){
            $model['search'] = 'byOutOfStock';
            $model['products'] = $productService->getProductByOutOfStock($products,$this->store);
        }else{
            $model['search'] = 'all';
            $model['products'] = $productService->countStocks($products,$this->store);
        }
                               ...
                               ProductService $productService): Response
    {
        ...
        $model['productStocks'] = $productService
            ->getProductStockNearExpirationDate((int) $model['dayBefore'],$this->store);
                              ...
                              ProductService $productService): Response
    {
        ...
        if ($request->get('status') !== 'withdraw'){
            ...
            $model['productStocks'] = $productService
                ->getProductStockOutdated(false,$this->store);
        }else{
                        ...
                        EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()){
            $productName = $productRepository
                ->getByNameAndCategory($product->getName(),$product->getCategory(),$this->store);
                           ...
                           SaleRepository $saleRepository): Response
    {
        ...
        $model['customers'] = $customerRepository
            ->findBy(['store' => $this->store,
                'type' => CustomerTypeConstant::TYPEKEYS['Reseller']]);

        $model['saleStats'] = $saleRepository
            ->groupByDateProduct($model['product'],$this->store);

        $model['salesYear'] = $saleRepository
            ->groupByYearProduct($model['product'],$this->store);
