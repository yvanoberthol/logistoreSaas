...
class ReportController extends AbstractController
{
                             ...
                             ProductRepository $productRepository): Response
    {
        ...
        $model['products'] = $productRepository->findBy(['store' => $this->store]);
        ...
        if ($request->isMethod('POST')){
            ...
            if ($product !== null){
                ...
                $model['productSales'] = $productSaleRepository
                    ->findProductSaleByGroupPeriod($model['start'],$model['end'],$product,$this->store);
                $model['losses'] = $lossRepository
                    ->findLossByPeriod($model['start'],$model['end'],$product,$this->store);
                $model['productStocks'] = $productStockRepository
                    ->findProductStockByPeriod($model['start'],$model['end'],$product,$this->store);
        ...
        if ($request->isMethod('POST')){
            ...
            if ($type !== null){
                $model['expenses'] = $expenseRepository
                    ->findExpenseByPeriod($model['start'],$model['end'],$type->getId(),$this->store);
            }else{
                $model['expenses'] = $expenseRepository
                    ->findExpenseByPeriod($model['start'],$model['end'],null,$this->store);
            }
                                ...
                                BankRepository $bankRepository): Response
    {
        ...
        $model['store'] = $this->store;
        $model['banks'] = $bankRepository->findBy(['store' => $this->store]);
        ...
        if ($request->isMethod('POST')){
            ...
            if ($bank !== null){
                ...
                $model['transactions'] = $transactionRepository
                    ->findTransactionByPeriod($model['start'],$model['end'],
                        $bank->getId(),$this->store);
                         ...
                         SaleRepository $saleRepository): Response
    {
        ...
        $model['sales'] = $saleRepository
            ->groupByPeriodDate($model['start'],$model['end'],null,$this->store);
                         ...
                         SaleRepository $saleRepository): Response
    {
        ...
        $model['sales'] = $saleRepository
            ->findSaleByPeriod($model['start'],$model['end'],null,
                false,false,$this->store);
                         ...
                         LossRepository $lossRepository): Response
    {
        ...
        $model['losses'] = $lossRepository
            ->findLossByPeriod($model['start'],$model['end'],null,$this->store);
                          ...
                          StockRepository $stockRepository): Response
    {
        ...
        $model['stocks'] = $stockRepository
            ->findStockByPeriod($model['start'],$model['end'],null,$this->store);
    ...
    public function saleByProduct(Request $request,
                          Pdf $pdf, SaleRepository $saleRepository,
                          ProductRepository $productRepository): Response
    {
        ...
        $model['products'] = $productRepository
            ->getByPeriodDate($model['start'],$model['end'],null,
                null,null,$this->store);

        $model['sales'] = $saleRepository
            ->findSaleByPeriod($model['start'],$model['end'],
                null,false,false,$this->store);
                                  ...
                                  ProductRepository $productRepository): Response
    {
        ...
        $model['products'] = $productRepository
            ->getByAdjustmentPeriodDate($model['start'],$model['end'],null,null,$this->store);
                                  ...
                                  ProductRepository $productRepository): Response
    {
        ...
        $products = $productRepository
            ->getSaleReturnByPeriodDate($model['start'],$model['end'],$this->store);

        $productStockables = $productRepository
            ->getSaleReturnStockableByPeriodDate($model['start'],$model['end'],$this->store);

        $productRepays = $productRepository
            ->getSaleReturnRepayByPeriodDate($model['start'],$model['end'],$this->store);
                                      ...
                                      ProductRepository $productRepository): Response
    {
        ...
        $products = $productRepository
            ->getStockReturnByPeriodDate($model['start'],$model['end'],$this->store);

        $productRepays = $productRepository
            ->getStockReturnRepayByPeriodDate($model['start'],$model['end'],$this->store);
                                  ...
                                  ProductRepository $productRepository): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $model['products'] = $productRepository
                ->getByPeriodDate($model['start'],$model['end'],null,
                    null,$model['category'],$this->store);
            ...
            if ($model['byProduct']){
                $model['products'] = $productRepository
                    ->getByPeriodDate($model['start'],$model['end'],
                        null,$model['customer'],null,$this->store);

                $sales = $saleRepository
                    ->findByPeriodCustomer($model['start'],$model['end'],
                        $model['customer'],null,$this->store);
                                          ...
                                          ProductRepository $productRepository): Response
    {
        ...
        $model['employees'] = $employeeRepository->findEmployees($this->store);
        ...
        if ($request->isMethod('POST')) {
            ...
            $model['products'] = $productRepository
                ->getByPeriodDate($model['start'],$model['end'], $model['employee'],
                    null,null,$this->store);

            $sales = $saleRepository
                ->findByPeriodUser($model['start'],$model['end'],
                    $model['employee'],false,$this->store);
                          ...
                          UserRepository $userRepository): Response
    {
        ...
        $model['salaries'] = $userRepository
            ->findByMonthYearReport($model['monthNow'],$model['year'],$this->store);
                           ...
                           EmployeeService $employeeService): Response
    {
        ...
        $model['attendances'] = $employeeService
            ->getReportAttendance($model['monthNow'],$model['year'],$this->store);
                                          ...
                                          ProductRepository $productRepository): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $model['products'] = $productRepository
                ->getStockByPeriodDate($model['start'],$model['end'],null,
                    $model['supplier'],null,$this->store);
                             ...
                             EncashmentRepository $encashmentRepository): Response
    {
        ...
        $model['employees'] = array_filter($userRepository->findEmployees($this->store), static function(User $user){
        ...
        if ($request->isMethod('POST')) {
            ...
            $encashments = $encashmentRepository
                ->findByPeriod($model['start'],$model['end'],$model['employee'],$this->store);
            ...
            use($encashmentService,$customerService,$customerRepository,$model){
                $customers= $customerService
                    ->getCredits($encashment->getDate(),$encashment->getDate(),
                        $customerRepository->findBy(['store' => $model['store']]),$model['employee']);
               ...
               $totalCredits = array_sum(array_map(static function($line){
                ...
                $file = $encashmentService
                    ->getInventory($encashment->getDate(),$encashment->getDate(),
                        $model['employee'],true,$model['store']);
