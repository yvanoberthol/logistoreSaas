...
class RestCartController extends AbstractController
{
    ...
    private function getInfoCart(): Response
    {
        ...
        $model['taxs'] = $this->taxRepository->findBy(['status' => true,'store' => $this->store]);
    ...
    private function getProductList(): array
    {
        ...
        $model['products'] = $this->productService->countStocks($products,$this->store);
    ...
    {
        return ProductDto
            ::createFromEntity($this->productRepository
                ->findOneBy(['qrCode' => $barcode,'store' => $this->store]));
    }
}
