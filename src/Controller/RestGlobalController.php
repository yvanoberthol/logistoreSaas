...
class RestGlobalController extends AbstractController
{
                                    ...
                                    ProductService $productService): Response
    {
        ...
        $products = $productService
            ->countStocks($productRepository->findBy(['store'=>$this->store]),$this->store);
                                ...
                                ProductStockRepository $productStockRepository): Response
    {
        ...
        $startDate = ($request->get('start')=== 'null')
            ?new DateTime():new DateTime($request->get('start'));
        $endDate = ($request->get('end')=== 'null')
            ?new DateTime():new DateTime($request->get('end'));
        ...
        $productStocks = $productSaleRepository
            ->findProductSaleByGroup($startDate,$endDate,$model['product'],
                $employee,$customer,$this->store);
                                     ...
                                     ProductSaleReturnRepository $productSaleReturnRepository): Response
    {
        ...
        $model['saleReturns'] = $productSaleReturnRepository
            ->findProductSaleReturnByGroup($startDate,$endDate,$model['product'],$this->store);
                                   ...
                                   ProductStockReturnRepository $productStockReturnRepository): Response
    {
        ...
        $model['stockReturns'] = $productStockReturnRepository
            ->findProductStockReturnByGroup($startDate,$endDate,$model['product'],$this->store);
    ...
    {
        $model['count'] = count($productRepository->findBy(['store'=>$this->store]));
        return $this->json($model,200,[]);
    }
    ...
    {
        $products = $productRepository->findBy(['store'=>$this->store]);

        $model['stockAlertCount'] = count($productService->getProductByStockAlert($products,$this->store));
        $model['outOfStockCount'] = count($productService->getProductByOutOfStock($products,$this->store));
        $model['stockOutOfDateCount'] = count($productService->getProductStockOutdated(false,$this->store));
        $model['stockExpiryDateCount'] = count($productService
            ->getProductStockNearExpirationDate($this->setting->getDaysBeforeExpiration(),$this->store));
        return $this->json($model,200,[]);
    }
                                     ...
                                     Request $request): Response
    {
        ...
        $model['salePayments'] = $salePaymentRepository
            ->findByPeriodDate($model['date'],$model['date'],null,
                $model['customer'],$this->store);
                                     ...
                                     Request $request): Response
    {
        ...
        $model['stockPayments'] = $stockPaymentRepository
            ->findStockPaymentByPeriod($model['date'],$model['date'],null,
                $model['supplier'],$this->store);
                                     ...
                                     Request $request): Response
    {
        ...
        $model['substitutes'] =
            $productService->countStocks($model['product']->getSubstitutes(),$this->store);
        if ($withStock){
