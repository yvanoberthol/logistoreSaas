...
class SaleController extends AbstractController
{
        ...
        foreach ($paymentMethods as $paymentMethod){
            $saleByMethod =
                $saleRepository->findSaleByMethodPeriod(
                    $model['start'],$model['end'],$paymentMethod,null,$this->store
                );
    ...
    {
        $model['sale'] = $sale;
        $model['paymentMethods'] = $paymentMethodRepository->findBy(['status' => true,'store'=>$this->store]);
        $model['customers'] = $customerRepository->findBy(['enabled' => true,'store'=>$this->store]);
                        ...
                        ProductService $productService): Response
    {
        ...
        if ($this->setting->getWithUserCategory()){
            ...
            $products = $productRepository->findBy(['enabled' => true,'store'=>$this->store],
                ['addDate' => 'DESC']);
        }
                        ...
                        ProductService $productService): Response
    {
        ...
        $model['products'] = $productService->countStocks($products,$this->store);
        $model['customers'] = $customerRepository
            ->findBy(['type' => CustomerTypeConstant::TYPEKEYS['Reseller'],'store'=>$this->store]);
        $model['entity'] = 'controller.sale.new.entity';
        $model['page'] = 'controller.sale.new.page';
     ...
     * @throws Exception
     */
    public function print(Sale $sale, Pdf $pdf): Response
    {
     ...
     * @throws Exception
     */
    public function invoicePrint(Sale $sale, Pdf $pdf): Response
    {
