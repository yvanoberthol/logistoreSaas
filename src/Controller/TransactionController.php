...
class TransactionController extends AbstractController
{
            ...
            if ($_POST['bank'] !== '0'){
                ...
                $model['transactions'] = $transactionRepository
                    ->findTransactionByPeriod($model['start'],$model['end'],
                        (int) $model['bankSearch'],$this->store);
            }else{
                $model['transactions'] = $transactionRepository
                    ->findTransactionByPeriod($model['start'],$model['end'],
                        $model['bankSearch'],$this->store);
            }
        }else{
            $model['transactions'] = $transactionRepository
                ->findTransactionByPeriod($model['start'],$model['end'],
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if (!GlobalConstant::compareDate($transaction->getDate(), new DateTime())){
                ...
                && $transaction->getTotalAmount() > $transaction->getBank()->getBalance()){
    ...
    public function edit(Transaction $transaction, Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($request->isMethod('POST')){
            $oldAmount = $transaction->getTotalAmount();

            $form->handleRequest($request);
            if ($form->isSubmitted() && $form->isValid()){
                if ($transaction->getType() === '0'
                    && $transaction->getTotalAmount() > ($transaction->getBank()->getBalance() + $oldAmount)){
                    $this->addFlash('danger',"controller.stockPayment.add.flash.danger3");
                }else{
                    $entityManager->persist($transaction);
                    $entityManager->flush();
