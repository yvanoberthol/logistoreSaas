...
class User implements UserInterface,PasswordAuthenticatedUserInterface,EquatableInterface,Serializable
{
     ...
     */
    public function getCategories(Store $store=null): Collection
    {

        return ($store === null)? $this->categories:
            array_filter($this->categories->toArray(),
                static function(ProductCategory $category) use($store){
                    return $category->getStore() === $store;
                });
    }
