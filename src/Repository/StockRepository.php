...
class StockRepository extends ServiceEntityRepository
{
    ...
    public function groupByDate($store=null): array
    {
    ...
    public function findStockByPeriod($start,$end,$product=null,$store=null)
    {
        $qb = $this->createQueryBuilder('s')
        ...
        if ($product !== null){
            $qb->innerJoin('s.productStocks','pst')
                ->andWhere('pst.product = :product')
    ...
    public function findByPeriodSupplier($start,$end,$supplier,$store=null)
    {
        $qb = $this->createQueryBuilder('s')
            ->where('s.status = true')
    ...
    public function groupByDateSupplier($supplier,$store=null): array
    {
        $qb = $this->createQueryBuilder('s')
            ->select('DATE(s.addDate) as date','count(s) as nbStocks',
                'sum(s.amount) as amount')
    ...
    public function groupByYearSupplier($supplier,$store=null): array
    {
        $qb = $this->createQueryBuilder('s')
    ...
    public function groupByPeriodDateSupplier($start,$end,$supplier,$store=null): array
    {
        $qb = $this->createQueryBuilder('s')
            ->select('DATE(s.addDate) as date', 'count(s) as nbStocks',
                'sum(s.amount) as amount')
    ...
    public function getStockByYearSupplier($year,$supplier= null,$store=null): ?array
    {
        $qb = $this->createQueryBuilder('s')
            ->select('DATE(s.addDate) as date','count(s) as nbStocks',
                'sum(s.amount) as amount')
    ...
    public function countAllBySupplier($supplier = null,$store=null)
    {
        try {
            $qb = $this->createQueryBuilder('s')
                ->select('count(s)')
