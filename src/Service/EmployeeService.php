...
class EmployeeService
{
    ...
    public function getReportAttendance($month, $year,$store=null): array {


        $employees = $this->userRepository->findEmployees($store);


        $sattendances = [];
        foreach ($employees as $employee){
            $sattendances[] = $this->getAttendanceLine($month, $year, $employee,$store);
        }

        return $sattendances;
    }

    private function getAttendanceLine($month, $year,User $employee,$store=null): array {

        $attendances = $this->attendanceRepository
            ->findByMonthYear($month,$year,$employee,$store);
