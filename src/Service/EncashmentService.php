...
class EncashmentService
{
    ...
    public function getInventory($start, $end, $employee = null,$enable=true,$store=null): array {
        ...
        if ($employee !== null && $this->setting->getWithUserCategory()){
            ...
            $products = $this->productRepository->findBy(['enabled'  => $enable,'store' => $store]);
        }

        $qtySolds = $this->productSaleRepository
            ->nbByProductAndPeriodDate($start,$end,null,$employee,$store);

        $qtySaleReturns = $this->productSaleReturnRepository
            ->nbByProductAndPeriodDate($start,$end,null,null,$employee,$store);
