...
class ProductService
{
     ...
     * @return array
     */
    public function getProductStockNearExpirationDate(int $days = 10,$store=null): array
    {
        $productStocks = $this->productStockRepository
            ->getAllNotWithDraw(abs($days),$store);
     ...
     * @return array
     */
    public function getProductStockOutdated(bool $withdraw,Store $store=null): array
    {
        $productStocks = $this->productStockRepository
            ->getOutdated($withdraw,$store);
    ...
    public function getInventory($start, $end, $byProduct=false,$employee = null,
                                 $enable=true,$store=null): array {


        if ($employee !== null && $this->setting->getWithUserCategory()){
            $products = [];
            foreach ($employee->getCategories($store) as $category){
            ...
            $products = $this->productRepository
                ->findBy(['enabled'  => $enable, 'store' => $store]);
        }

        $stockCounts = $this->productStockRepository
            ->nbByProductAndBeforePeriodDate($start,null,$store);
     ...
     * @throws Exception
     */
    public function countStocks($products,$store=null): array {

        $productStockCounts = $this->productStockRepository->countByProducts($store);
        $productSaleCounts = $this->productSaleRepository->countByProducts($store);
        $productAdjustCounts = $this->productAdjustRepository->countByProducts($store);
        $productSaleReturnCounts = $this->productSaleReturnRepository->countByProducts($store);
        $lossCounts = $this->lossRepository->countByProducts($store);

        $productStockReturnCounts = $this->productStockReturnRepository->countByProducts($store);
     ...
     * @throws Exception
     */
    public function getProductByStockAlert($products,Store $store=null): ?array {

        $productFilter = array_filter($this->countStocks($products,$store), static function(Product $product){
     ...
     * @throws Exception
     */
    public function getProductByStockNotFinish($products,Store $store=null): ?array {

        $productFilter =  array_filter($this->countStocks($products,$store),
     ...
     * @throws Exception
     */
    public function getProductByOutOfStock($products,Store $store=null): ?array {
    ...
    public function getProfitByProduct(Product $product, int $qty): float {
        ...
        $productStocks =
            $this->getProductStockDispoByProduct($product,10);
